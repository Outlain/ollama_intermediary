...
export class AutomaticRecovery {
  ...
  restore() {
    try {
      ...
      if (data.episode && (typeof data.episode.key !== 'string' || data.episode.key.length > 128
        || !Number.isSafeInteger(data.episode.attempts) || data.episode.attempts < 0)) throw new Error('invalid_episode');
  ...
  async runStep() {
    ...
    const verifying = current && ['verifying', 'completed'].includes(current.phase);
  ...
  async performRestart(operation) {
    ...
    this.manualRequested = false;
    this.persist();
    this.transition(operation.phase === 'failed' && this.saved.episode.attempts < this.settings.max_restarts
      ? 'cooldown' : 'needs_attention', this.lastError);
  }
